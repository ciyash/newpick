...
export const getFixturesByDateRange = async (req, res) => {
  try {
    ...
    /* ─── Format Response ─── */
    const formatted = fixtures.map(fixture => {
      ...
      return {
        ...
        score: {
          home: fixture.scores?.find(s => s.description === "CURRENT" && s.score?.participant === "home")?.score?.goals ?? null,
          away: fixture.scores?.find(s => s.description === "CURRENT" && s.score?.participant === "away")?.score?.goals ?? null,
        }
      };
    });
